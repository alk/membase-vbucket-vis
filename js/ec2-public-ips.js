...
function reloadData() {
  ...
  InjectionController.slaveApply(onResult, function (reply) {
    ...
    var code = 'S = "{json, [rpc:call(N, erlang, apply, [fun () -> {struct, [{nodeName, N}, {masterNode, (try rpc:call(N, mb_master, master_node, []) of {badrpc, _} -> unknown; X -> X catch _:_ -> unknown end)}, {publicIP, list_to_binary(os:cmd(\\"curl -s http://169.254.169.254/latest/meta-data/public-ipv4\\"))}]} end, []]) || N <- [node() | nodes()]]}.",' +
